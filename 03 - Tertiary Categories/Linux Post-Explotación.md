---
aliases:
  - Linux Post-Explotación
tags:
  - env/linux
primary categories:
  - "[[Red Team]]"
secondary categories:
  - "[[Post-Explotación]]"
  - "[[Linux]]"
kind: Tertiary Category
---
---
# Linux Post-Explotación
Enumeración inicial y Conciencia Situacional.

## System & Network Information

   - [[Linux - System & Kernel Enumeration]]
   - [[Linux - Network & Interface Enumeration]]
   - [[Linux - Security & AV Identification]]

## Filesystem & Internals

   - [[Linux - File System Enumeration]]
   - [[Linux - Installed Software Enumeration]]
   - [[Linux - Process & Service Enumeration]]
   - [[Linux - User & Group Enumeration]]

---

# ⬆ Linux Privilege Escalation
Técnicas de elevación de privilegios.

## Sudo & User Rights Abuse
  Abuso de derechos legítimos mal configurados.
  
   - [[Abusing Sudo Rights]]
       - [[Exploiting LD_PRELOAD & LD_LIBRARY_PATH]]
       - [[Exploiting Vulnerable Sudo Versions]]
   - [[Abusing Linux Capabilities]]

## File Permission & Binary Abuse
  Explotación de permisos de archivos inseguros y binarios con flags especiales.
  
   - [[Abusing SUID & SGID Binaries]]
   - [[Abusing Weak File Permissions]]
       - [[Exploiting Writeable etc-passwd]]
       - [[Exploiting Writeable etc-shadow]]

## Environment & Path Manipulation
  Manipulación del entorno de ejecución para secuestrar llamadas.
  
   - [[Exploiting Cron Jobs]]
   - [[Exploiting PATH Hijacking]]
   - [[Exploiting Wildcard Injection]]
   - [[Exploiting Shared Object Injection]]
   - [[Exploiting Python Library Hijacking]]

## Dangerous Groups Abuse
  Si el usuario pertenece a uno de estos grupos, es casi root.
  
   - [[Abusing Dangerous Groups]]
       - [[Abusing Docker Group]]
       - [[Abusing LXC-LXD Group]]
       - [[Abusing Disk Group]]
       - [[Abusing Shadow Group]]
       - [[Abusing Adm Group]]

## Application & Kernel Exploits
  Vulnerabilidades en servicios específicos o el kernel.
  
   - [[Exploiting NFS Misconfigurations]]
   - [[Abusing Tmux Sessions]]
   - [[Linux Kernel Exploits]]
       - [[Exploiting PwnKit (CVE-2021-4034)]]
   - [[Linux PrivEsc Payloads]]
  ---

# 🔐 Credential Dumping & Looting
Robo de secretos.

## Hash & Secret Extraction
   - [[Linux Credential Hunting]]
   - [[Extracting & Cracking Shadow Hashes]]

---
